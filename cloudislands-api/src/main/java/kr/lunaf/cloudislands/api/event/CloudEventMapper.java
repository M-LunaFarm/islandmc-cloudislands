...
public final class CloudEventMapper {
    ...
    public static Optional<CloudEvent> map(GlobalEventSnapshot snapshot) {
        ...
        return switch (snapshot.type()) {
            ...
            case "ISLAND_MIGRATED" -> Optional.of(new IslandMigratedEvent(uuid(fields, "islandId"), text(fields, "fromNode"), firstText(fields, "toNode", "targetNode"), text(fields, "worldName"), intValue(fields, "cellX"), intValue(fields, "cellZ"), longValue(fields, "fencingToken"), occurredAt));
            ...
            case "ROUTE_TICKET_FAILED" -> Optional.of(new RouteTicketFailedEvent(uuid(fields, "ticketId"), uuid(fields, "islandId"), uuid(fields, "playerUuid"), text(fields, "action"), text(fields, "targetNode"), text(fields, "requestedNode"), text(fields, "reason"), occurredAt));
