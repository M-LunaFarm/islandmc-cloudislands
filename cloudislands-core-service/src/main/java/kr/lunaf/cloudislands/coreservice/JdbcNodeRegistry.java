...
public final class JdbcNodeRegistry implements NodeRegistry {
    ...
    public void heartbeat(NodeHeartbeatRequest request) {
        ...
        try (Connection connection = dataSource.getConnection();
            PreparedStatement statement = connection.prepareStatement(heartbeatUpsertSql(connection))) {
            statement.setString(1, request.nodeId());
            statement.setString(2, NodeRegistry.safeHeartbeatPool(request));
            statement.setString(3, NodeRegistry.safeHeartbeatVelocityServerName(request));
    ...
    private String heartbeatUpsertSql(Connection connection) throws SQLException {
        String insert = "INSERT INTO server_nodes(id, pool, velocity_server_name, node_version, state, soft_player_cap, hard_player_cap, reserved_slots, max_active_islands, players, active_islands, mspt, heap_used_mb, heap_max_mb, activation_queue, max_activation_queue, chunk_load_pressure, recent_failure_penalty, object_storage_available, supported_templates, last_heartbeat, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), now())";
        if (mysqlLike(connection)) {
            return insert + " ON DUPLICATE KEY UPDATE pool = VALUES(pool), velocity_server_name = VALUES(velocity_server_name), node_version = VALUES(node_version), state = VALUES(state), soft_player_cap = VALUES(soft_player_cap), hard_player_cap = VALUES(hard_player_cap), reserved_slots = VALUES(reserved_slots), max_active_islands = VALUES(max_active_islands), players = VALUES(players), active_islands = VALUES(active_islands), mspt = VALUES(mspt), heap_used_mb = VALUES(heap_used_mb), heap_max_mb = VALUES(heap_max_mb), activation_queue = VALUES(activation_queue), max_activation_queue = VALUES(max_activation_queue), chunk_load_pressure = VALUES(chunk_load_pressure), recent_failure_penalty = VALUES(recent_failure_penalty), object_storage_available = VALUES(object_storage_available), supported_templates = VALUES(supported_templates), last_heartbeat = now(), updated_at = now()";
        }
        return insert + " ON CONFLICT (id) DO UPDATE SET pool = EXCLUDED.pool, velocity_server_name = EXCLUDED.velocity_server_name, node_version = EXCLUDED.node_version, state = EXCLUDED.state, soft_player_cap = EXCLUDED.soft_player_cap, hard_player_cap = EXCLUDED.hard_player_cap, reserved_slots = EXCLUDED.reserved_slots, max_active_islands = EXCLUDED.max_active_islands, players = EXCLUDED.players, active_islands = EXCLUDED.active_islands, mspt = EXCLUDED.mspt, heap_used_mb = EXCLUDED.heap_used_mb, heap_max_mb = EXCLUDED.heap_max_mb, activation_queue = EXCLUDED.activation_queue, max_activation_queue = EXCLUDED.max_activation_queue, chunk_load_pressure = EXCLUDED.chunk_load_pressure, recent_failure_penalty = EXCLUDED.recent_failure_penalty, object_storage_available = EXCLUDED.object_storage_available, supported_templates = EXCLUDED.supported_templates, last_heartbeat = now(), updated_at = now()";
    }
