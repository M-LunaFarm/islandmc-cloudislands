...
CREATE TABLE IF NOT EXISTS island_warps (
    ...
    CONSTRAINT chk_island_warps_world_not_blank CHECK (trim(world_name) <> ''),
    CONSTRAINT chk_island_warps_world_trimmed CHECK (world_name = trim(world_name)),
...
CREATE TABLE IF NOT EXISTS island_warehouse (
    ...
    CONSTRAINT fk_island_warehouse_island FOREIGN KEY (island_id) REFERENCES islands(id),
...
CREATE TABLE IF NOT EXISTS island_missions (
    ...
    updated_by CHAR(36),
    updated_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    PRIMARY KEY (island_id, mission_key, kind),
...
CREATE TABLE IF NOT EXISTS island_mission_definitions (
...
CREATE INDEX idx_island_mission_definitions_provider ON island_mission_definitions(provider_id, enabled, mission_key);
