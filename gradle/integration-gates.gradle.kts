...
tasks.register("verifyIntegrationRuntimeSmoke") {
    ...
    val craftEngineFurnitureBridge = layout.projectDirectory.file("cloudislands-paper/src/main/java/kr/lunaf/cloudislands/paper/integration/customitem/CraftEngineFurnitureBridge.java")
    val craftEngineFurnitureBridgeTest = layout.projectDirectory.file("cloudislands-paper/src/test/java/kr/lunaf/cloudislands/paper/integration/customitem/CraftEngineFurnitureBridgeTest.java")
    ...
    inputs.files(certification, certificationTest, registry, planRuntime, planRuntimeTest, visibilityRuntime, visibilityRuntimeTest, customBlockRuntime, customBlockRuntimeTest, craftEngineFurnitureBridge, craftEngineFurnitureBridgeTest, stackAmountRuntime, stackAmountRuntimeTest)
    doLast {
        ...
        val craftEngineFurnitureSource = craftEngineFurnitureBridge.asFile.readText()
        val craftEngineFurnitureTests = craftEngineFurnitureBridgeTest.asFile.readText()
        ...
        val failures = buildList {
            ...
            if (!customBlockRuntimeSource.contains("CraftEngineFurniture") || !customBlockRuntimeSource.contains("getLoadedFurnitureByMetaEntity")) add("Custom entity reconciliation must resolve CraftEngine furniture IDs")
            ...
            if (!craftEngineFurnitureSource.contains("FurniturePlaceEvent") || !craftEngineFurnitureSource.contains("FurnitureBreakEvent")) add("CraftEngine furniture must have optional place and break event bridges")
            if (!craftEngineFurnitureSource.contains("IslandPermission.BUILD") || !craftEngineFurnitureSource.contains("IslandPermission.BREAK")) add("CraftEngine furniture events must enforce island permissions")
            if (!craftEngineFurnitureSource.contains("customEntityPlaced") || !craftEngineFurnitureSource.contains("customEntityRemoved")) add("CraftEngine furniture events must update incremental value accounting")
            if (!craftEngineFurnitureTests.contains("officialPlaceAndBreakPayloadsResolveOneStableCustomKey")) add("CraftEngine furniture event payload regression coverage is missing")
