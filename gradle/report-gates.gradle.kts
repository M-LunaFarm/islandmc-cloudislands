...
private fun featureParityEntries(): List<FeatureParityEntry> = listOf(
    FeatureParityEntry(
        ...
        "ciIntegrationSmoke verifies cross-Core create, job, route, session, consume, and player-ticket cache convergence; Paper tests verify main-thread template permission preflight, target-island coordinates, safe destination scans, and final bounded destination revalidation",
    ...
    FeatureParityEntry(
        ...
        "verifyMissionEventProgress covers final uncancelled block, farm, kill, fishing, capacity-bounded bulk crafting, enchanting, statistic, advancement, and item-consumption progress plus the bounded definition cache; reward-settlement tests cover failure reopening, repeatable reset, and durable warehouse item delivery; a shared Core idempotency receipt ledger protects bank, warehouse, upgrade, and other POST/PUT/PATCH/DELETE mutation retries; Paper warehouse policy rejects metadata-bearing items that its material-and-amount schema cannot restore, while overflow-safe logical-stack mob-drop scaling, upgrade CAS/refund, generator, and economy safety gates cover the remaining scope",
        "not recovery-specific",
        listOf(
    ...
    FeatureParityEntry(
        ...
        "verifyReviewModerationCoverage plus Core audit/visitor route tests and LOWEST/HIGHEST mutually exclusive local/team-chat isolation cover current workflow",
