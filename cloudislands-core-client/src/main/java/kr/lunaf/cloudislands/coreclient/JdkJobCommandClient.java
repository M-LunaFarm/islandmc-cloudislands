...
final class JdkJobCommandClient implements JobCommandClient {
    ...
    public CompletableFuture<JobActionView> retry(UUID jobId) {
        ...
        return core.postResultBody("/v1/admin/jobs/retry", CoreJsonPayload.object("jobId", jobId))
    ...
    public CompletableFuture<JobActionView> cancel(UUID jobId) {
        requireId(jobId, "jobId");
        return core.postResultBody("/v1/admin/jobs/cancel", CoreJsonPayload.object("jobId", jobId))
            .thenApply(CoreResponseBody::value)
    ...
    public CompletableFuture<JobRecoveryView> recover(String nodeId, long minIdleMillis, int maxJobs) {
        return core.postResultBody("/v1/admin/jobs/recover", CoreJsonPayload.object("nodeId", requireJobNode(nodeId), "minIdleMillis", Math.max(0L, minIdleMillis), "maxJobs", Math.max(1, maxJobs)))
            .thenApply(CoreResponseBody::value)
