...
final class IslandChatLogCommandHandler {
    ...
    boolean handleCommand(Player player, String subcommand, String[] args) {
        ...
        if (subcommand.equals("teamchat") || subcommand.equals("team-chat") || subcommand.equals("teamchat-toggle") || subcommand.equals("tc") || subcommand.equals("팀채팅")) {
            if (args.length < 2) {
                toggleTeamChatMode(player);
                return true;
            }
            if (isChatModeArgument(args[1])) {
                updateTeamChatMode(player, args[1]);
                return true;
            }
    ...
    private void toggleTeamChatMode(Player player) {
        if (teamChatModes.enabled(player.getUniqueId())) {
            setTeamChatMode(player, teamChatModes.toggle(player.getUniqueId()));
            return;
        }
        runtime.currentIsland(player, message("chat-team-required", "섬 안에서만 팀 채팅을 사용할 수 있습니다."))
            .ifPresent(_islandId -> setTeamChatMode(player, teamChatModes.toggle(player.getUniqueId())));
    }

    private void updateTeamChatMode(Player player, String argument) {
        UUID playerUuid = player.getUniqueId();
        if (disabledArgument(argument)) {
            setTeamChatMode(player, teamChatModes.set(playerUuid, false));
            return;
        }
        ...
        runtime.currentIsland(player, message("chat-team-required", "섬 안에서만 팀 채팅을 사용할 수 있습니다."))
            .ifPresent(_islandId -> setTeamChatMode(player, teamChatModes.set(playerUuid, true)));
    }
    ...
    private void sendChat(Player player, String channel, String chatMessage, String labelKey, String labelFallback, String missingKey, String missingFallback) {
        ...
        runtime.currentIsland(player, message(missingKey, missingFallback)).ifPresent(islandId -> {
            ...
            communicationUseCase.sendChatAction(islandId, playerSession.playerUuid(), channel, chatMessage, runtime::mutate)
                .thenAccept(result -> {
                    if (!result.accepted()) {
                        deliverMessage(playerSession, label + message("chat-send-failed-suffix", "을 전송하지 못했습니다."));
                        return;
                    }
                    deliverMessage(playerSession, label + message("chat-send-success-suffix", "을 전송했습니다."));
                })
                .exceptionally(error -> {
                    deliverMessage(playerSession, label + message("chat-send-failed-suffix", "을 전송하지 못했습니다."));
                    return null;
    ...
    private void listLogs(Player player, int limit) {
        ...
        runtime.currentIsland(player, message("log-list-island-required", "섬 안에서만 로그를 확인할 수 있습니다.")).ifPresent(islandId -> {
            communicationUseCase.logViews(islandId, limit)
                .thenAccept(logs -> deliverMessage(playerSession, logListMessage(logs)))
                .exceptionally(error -> {
                    deliverMessage(playerSession, message("log-list-load-failed", "섬 로그를 불러오지 못했습니다."));
                    return null;
                });
        });
    }
