...
public final class CropGrowthLevelCache {
    ...
    public int level(UUID islandId) {
        ...
        client.listIslandUpgrades(islandId).thenAccept(body -> cache.put(islandId, new CachedLevel(parseCropLevel(body), System.currentTimeMillis() + TTL_MILLIS)));
    ...
    private int parseCropLevel(String json) {
        ...
        int marker = json.indexOf("\"upgradeKey\":\"crop\"");
        if (marker < 0) {
            return 1;
        }
