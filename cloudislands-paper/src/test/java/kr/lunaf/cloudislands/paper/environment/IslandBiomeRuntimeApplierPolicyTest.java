...
class IslandBiomeRuntimeApplierPolicyTest {
    ...
    void islandNodesApplyBiomeEventsInCancellableChunkBatches() throws Exception {
        ...
        assertTrue(bootstrap.contains("new kr.lunaf.cloudislands.paper.platform.world.IslandBiomeRuntimeApplier(plugin, plugin.agent.protection())"));
        assertTrue(applier.contains("onBiomeChange(IslandBiomeChangeEvent event)"));
        assertTrue(applier.contains("generations.merge(event.islandId(), 1L, Long::sum)"));
