...
Mission progress distinguishes incremental gameplay counters from authoritative
values. Block, crafting, combat, fishing, statistic, advancement, and consume
events use replay-safe idempotency keys so the Core client can retry a lost HTTP
response without double credit. Bank-balance missions advance to the current
Core balance instead of adding that balance again after every deposit, and
level recalculation advances `island_level_10` to the returned level. Absolute
progress is monotonic: a later withdrawal or lower recalculation cannot erase
already earned progress, while repeated reports of the same value do not inflate
it.
...
`itemsadder:namespace:machine`. CraftEngine IDs retain their configured
namespace, for example `craftengine:default:palm_log`, and resolve from the
stable `CraftEngineBlocks.getCustomBlockState(Block)` API without constructing
a synthetic world-less location. CraftEngine furniture resolves through
`CraftEngineFurniture.getLoadedFurnitureByMetaEntity(Entity)`; its official
place and break events enforce island build/break permissions and update the
same `craftengine:<id>` key immediately after successful operations. Oraxen
and Nexo furniture use the same key as their matching item. For example, an operator can run
...
coverage; Paper 26.2 build 60 beta is also compile- and boot-verified on Java 25,
but remains experimental and is not reported as release-supported or domain
...
| access/bans/membership/roles/permissions | IMPLEMENTED_VERIFIED | Core API and permission event replay are exercised in tests; accepted visitor bans and kicks return to the Paper scheduler and evict the target independently from actor connectivity, while delayed actor feedback requires the exact initiating Player connection; permission and role queries, direct mutations, override resolution, and staged-save success or conflict UI retain that same exact connection, and an older save completion cannot clear equal-valued changes staged by a replacement connection | third-party permission plugins are integration-status reported, not all boot-verified |
...
- the shared asynchronous `/ciadmin` command runner now captures the initiating
  Player connection before Core, filesystem, config, or diagnostic work begins
- delayed success and failure messages return through the Paper scheduler and
  are delivered only when that exact Player instance is still online
- a disconnected operator's old callback can no longer target a replacement
  connection with the same UUID, while console senders retain normal feedback
- focused policy coverage certifies both branches for every admin operation that
  uses the shared runner, including config, migration, node, storage, and audit
...
- personal island flight changes now reserve a Core-authoritative preference
  revision, so the newest choice wins across multiple Paper and Core servers
- PostgreSQL and MySQL atomically reject stale flight writes with
  `PLAYER_PREFERENCE_SUPERSEDED` instead of overwriting a newer preference
- delayed callbacks retain the exact Player connection and a per-update token,
  preventing an old reconnect from applying flight or unlocking its replacement
- a bounded re-reservation closes the delayed-reservation edge case, while
  focused ordering tests and dual-Core PostgreSQL/MySQL smoke evidence certify
  the behavior
...
- delayed personal world-border responses now require the exact initiating
  Player connection, the same current island, and the latest request revision
- moving to another island or lobby while profile, flag, size, or color data is
  loading can no longer reapply the previous island's border afterward
- same-UUID reconnects and out-of-order visibility or color responses cannot
  overwrite a newer border request on the active connection
- focused connection, island, and ordering tests plus the complete Paper and
  personal-visual-preference gates certify the live border boundary
...
- local home and warp commands now retain the exact Player connection through
  Core lookup, public-access and permission resolution, safe-destination lookup,
  final teleport, single-Paper fallback movement, and delayed feedback
- disconnecting and reconnecting with the same UUID can no longer inherit a
  local home/warp teleport or result started by the previous connection
- legacy warp target resolution and every delayed home/warp mutation, listing,
  access-control response, and message use the same connection-session fence
- focused reconnect policy coverage, feature-parity evidence, and the complete
  Paper test suite certify the local teleport boundary

Release notes for `v1.1.241`:

- delayed warehouse list, prepare, escrow, mutation, grant, refund, PDC, and
  cleanup callbacks now retain the exact Player connection that initiated them
- disconnecting and reconnecting with the same UUID can no longer make an older
  request remove from, add to, or rewrite the replacement Player's inventory
- once an operation has a durable PDC/Core settlement, a replacement connection
...
- paid island creation now retains the exact Player connection through template
  lookup, permission preflight, economy charge, Core creation, and terminal UI
- if the connection closes or is replaced after a successful charge, the cost
  is refunded before any Core island-create mutation can start
- create, delete, and reset feedback can no longer cross into a later connection
  that happens to have the same UUID
- routing and lifecycle operations now share `PlayerConnectionSession`, with
  focused identity, charge/refund ordering, and complete Paper test coverage

Release notes for `v1.1.239`:

- `/ciadmin island teleport` now retains the exact Player connection that
  initiated the administrator route throughout ticket creation and polling
- route-session publication, BungeeCord transfer, and failure feedback each
  revalidate the initiating connection on Paper's scheduler
- disconnecting or reconnecting with the same UUID clears the old ticket with
  `PLAYER_SESSION_REPLACED` before another polling or handoff transition
- focused administrator routing policy coverage and the complete Paper test
  suite certify the same reconnect boundary as normal player routing

Release notes for `v1.1.238`:

- home, warp, and visit routing now retains the exact Player connection that
  initiated the request through ticket creation, polling, publication, local
  consumption, world loading, safe-destination resolution, and final teleport
- disconnecting and reconnecting with the same UUID can no longer inherit an
  older teleport, fallback move, failure message, loading bar, or delayed kick
- a proxy route published just as its source session is replaced is cleared
  with `PLAYER_SESSION_REPLACED` instead of being handed to the new connection
- focused identity and source-policy tests cover every delayed routing boundary,
  while the complete Paper test suite remains green
...
- migration return tickets now retain the exact Player instance that initiated
  the request throughout the bounded Core polling lifecycle
- a disconnect or same-UUID reconnect cancels route-session publication,
  BungeeCord transfer, failure feedback, and delayed BossBar cleanup
- READY tickets are revalidated on Paper's scheduler before any route session
  is published, preventing stale migration state from reaching Core
- focused identity and threading tests cover replacement, offline, mismatched,
  publication-preflight, transfer, feedback, and presentation boundaries

Release notes for `v1.1.235`:

- delayed team and island chat failures now retain the exact Player session that
  initiated the Core request
- disconnecting and reconnecting with the same UUID can no longer receive a
  failure message from the previous session
- failure feedback still returns through Paper's scheduler and re-resolves the
  active player before touching Bukkit state
- focused policy coverage locks both chat modes to the shared same-instance
  reconnect fence, while the complete Paper test suite remains green
...
- delayed island-boundary recovery now re-resolves the current online Player on
  Paper's scheduler before any message or teleport
- a disconnect/reconnect with the same UUID can no longer inherit an older
  pending boundary return through a stale Player instance
- movement, command teleports, or other world changes while the safe destination
...
- async team and island chat now re-resolve the same online Player instance on
  Paper's scheduler before reading location or sending through Core
- disconnecting or reconnecting between the async event and scheduled delivery
  can no longer replay queued chat through a stale or replacement Player
- global admin-spy permission checks and messages now run on the Paper thread and
  observe only final uncancelled chat at MONITOR priority
- focused identity-policy tests cover online, offline, stale, and same-UUID
  reconnect boundaries while existing isolation and command gates remain green

Release notes for `v1.1.231`:

- Paper generator selection now enforces each Core rule's minimum island level
  as well as its generator upgrade level and biome requirement
- the generator hot-path cache loads the authoritative Core island level without
  blocking the Bukkit thread; unavailable level data fails closed at level zero
- island level update events invalidate generator selections immediately, so
  newly unlocked materials do not wait for the cache TTL
- focused tests cover level-gated eligibility, authoritative level propagation,
  unavailable-level fallback, and the generator release evidence gate

Release notes for `v1.1.230`:

- entities permanently removed through another plugin's `Entity#remove()` path
  now decrement both aggregate and exact entity-type island counters
- Paper `PLUGIN` and `DISCARD` removal causes are reconciled immediately instead
  of leaving limits falsely occupied until a manual or periodic rescan
- chunk unloads, player-quit vehicle detaches, deaths, explosions, and direct
  hit removals remain excluded from this path because they persist or are
  already accounted by their dedicated accepted event
- focused policy tests and the release parity gate preserve the removal-cause
  split across supported Paper runtimes
...
- bank-balance missions now advance to the authoritative Core balance instead
  of adding the full balance again after every successful deposit
- island level recalculation now advances the built-in level mission with the
  returned authoritative level
- absolute mission progress is monotonic and repeat-safe, so stale lower values
  cannot regress progress and repeated equal values cannot inflate it
- automatic gameplay and metric progress delivery now carries stable
  idempotency metadata, enabling safe transport retries without duplicate credit
...
- concurrent team, warp, home, coop, and role limits plus crop growth, mob drop,
  spawner rate, and island potion effects are written to the authoritative Core
  keys already consumed by Paper
- rate fields use the documented SS2 multiplier unit, so values such as
  `crops-growth: 1.4` become a 140-percent runtime rate instead of 1 percent
- both native `levels:` files and SS2-style quoted levels without that wrapper
  are accepted; `cost:` and `price:` are equivalent, and nested maps are retained
- custom composite upgrades without a matching primary field no longer inherit
  an unrelated fallback size increase
- every configured capacity or modifier uses monotonic writes, preserving larger
  values previously earned from missions or set by administrators

Example `CI_UPGRADES_FILE` entry:
...
Release notes for `v1.1.222`:

- overview, environment, warehouse-list, administrator, home, warp, and routing
  callbacks capture immutable player UUIDs and re-resolve the current online
  Paper player before feedback, inventory access, permission checks, or movement
- stacked-block and personal-border toggles now compose profile reads and writes
  into one observable completion chain, so write failures cannot escape through
  an unobserved nested future
- all revision-guarded GUI loaders require the same current online Player
  instance as the reserved session; disconnect, reconnect, close, and navigation
  races discard late responses instead of opening stale inventories
- home/warp routing keeps Core lookup stages in one future chain, and local safe
  destination completion performs a final UUID-based player revalidation before
  teleporting or sending its result
- route-ticket polling, loading bars, local single-Paper consumption, published
  sessions, fallback movement, and Bungee plugin messages no longer retain a
  Player instance across delayed work

Release notes for `v1.1.221`:

- Paper setting mutations distinguish command and GUI flows: commands no longer
  force-open a delayed menu, while GUI toggles expose saving state and only
  return to settings when their original GUI session is still current
- setting, locale, flag, lifecycle, bank, and snapshot callbacks capture UUIDs,
  return to the Paper scheduler, and re-resolve the current online player before
  sending feedback or touching Bukkit UI state
- island create permission checks re-resolve the player after template lookup;
  disconnects and reconnects cannot charge or report through a stale Player
  instance
- bank balance, target lookup, deposit, withdrawal, refund/rollback outcomes,
  and snapshot create/restore results now share the same scheduler-safe delivery
  boundary, with source policy tests preventing captured-player regressions
...
- personal border color and visibility responses return to the global scheduler,
  ...
  instances
- failed team and island chat deliveries also return to the Paper scheduler and
  re-resolve the sender by UUID before notifying them, while asynchronous admin
  lookup failures use the same scheduler-bound delivery boundary
...
- deactivation now retains protection, active-island registration, and cell
  ...
  chunk in the island range is verified unloaded
- a player, plugin ticket, missing world, timeout, or unload refusal fails the
  deactivation without releasing its cell for another island
- bundle-less templates, including the built-in default template, now clear the
  reused cell's block, entity, and POI region files transactionally instead of
  exposing data from the previous island
- after the clean cell is published, Paper prepares the center chunk
  asynchronously and creates a safe dependency-free grass, dirt, bedrock, and
  sapling starter island under the configured template spawn
- starter block writes run only through the Bukkit platform adapter and global
  scheduler, and the existing fresh `CREATED` world flush captures the generated
  island in its first durable snapshot
- regression coverage verifies save-before-unload-before-release ordering,
  transition cleanup, bounded cell geometry, all three stale region datasets,
  neighboring-cell preservation, starter coordinates, and scheduler boundaries

Release notes carried forward from `v1.1.211`:
...
- the generated player feature-parity report now classifies all nine audited
  ...
  from compatibility-only adapters and intentional security restrictions
- Vault, PlaceholderAPI, Plan, vanish, custom-content, and stack-accounting
  integrations remain backed by executable runtime services and smoke evidence
- CoreProtect stays append-only and WorldEdit/FAWE stay compatibility-only:
  CloudIslands chunk bundles remain the sole authority for world-state transfer,
  avoiding conflicting or incomplete restore sources
- formatting-only MiniMessage intentionally keeps click, URL, insertion,
  selector, score, and NBT tags disabled at the untrusted-format boundary
- the complete 182-task clean check and Paper 26.1.2 boot smoke passed

Release notes carried forward from `v1.1.201`:

- mission persistence now uses `(missionKey, kind)` throughout in-memory,
  PostgreSQL, and MySQL storage, so a MISSION and CHALLENGE can safely share a
  key without overwriting definitions or progress
- PostgreSQL V86 and MySQL/MariaDB V10 upgrade existing primary keys; the MySQL
  migration also restores mission metadata columns that were absent from the
  consolidated schema and could make addon mission registration fail
- MySQL mission progress now computes completion before its left-to-right
  progress assignment, preventing partial progress from becoming an invalid
  `completed=true` row and failing the request
- dual-Core PostgreSQL, upgraded MySQL, and clean-install MySQL integration
  smokes all verified independent same-key progress; the complete 182-task
  check passed

Release notes carried forward from `v1.1.200`:

- Paper's dedicated item-frame change event now enforces `ITEM_FRAME` for add,
  rotate, and remove actions on normal and glow item frames
- item pickup policy now runs at both the Paper attempt event and the final
  entity pickup event at `HIGHEST`, closing early-event and downstream-plugin
  bypass paths through the same origin-island PDC authorization decision
- administrator bypass, member/owner permissions, visitor pickup flags, and
  migration fail-closed behavior retain their existing policy
- Paper 1.21.11, 26.1.2, and 26.2 compile coverage, Paper 26.1.2 boot smoke,
  the expanded protection smoke, and the complete 182-task check passed

Release notes carried forward from `v1.1.199`:
...
- player pickup checks use the origin island's cached role and
  `PICKUP_ITEM`/`VISITOR_PICKUP` policy instead of trusting the item's current
  wilderness or neighboring-island location
- hoppers cannot absorb an origin-tagged item outside its source island, and
  protected items cannot merge with wilderness or different-island drops to
  erase ownership metadata
- migration state remains fail-closed, administrator bypass remains explicit,
  and accepted owner/member/visitor-flag pickups preserve existing behavior
- targeted origin/merge/destination regressions, the expanded protection smoke,
  the complete 182-task check, and the 199-task release build passed

Release notes carried forward from `v1.1.196`:

- MySQL/MariaDB connections now force a UTC database session and use UTC
  calendars for every implicit JDBC timestamp bind/read, preventing fresh
  heartbeats and leases from appearing nine hours stale on Asia/Seoul hosts
- schema bootstrap no longer mistakes MySQL `42000` syntax errors containing
  the word `EXISTS` for harmless duplicate-object errors
- MySQL V6 now uses valid portable DDL, while V9 repairs installations where
  the old V6 was incorrectly recorded as applied without `island_fly_enabled`
- CI now runs the complete two-Core Redis/MinIO integration scenario against
  both PostgreSQL and MySQL 8.4, including creation, jobs, routing, recovery,
  bank, membership, warps, event replay, audit visibility, and database backup
- clean and previously affected MySQL databases, targeted regressions, and the
  complete 182-task check passed

Release notes carried forward from `v1.1.195`:
...
- bucket empty/fill, fertilization, natural structure growth, Enderman block
  ...
  now publish value, level, ranking, and limit deltas only at the final
  uncancelled `MONITOR` result
- a later protection or integration plugin can therefore cancel any of those
  mutations without leaving CloudIslands counts different from the world
- Paper's `BlockSpreadEvent -> BlockFormEvent -> BlockGrowEvent` inheritance is
  handled as mutually exclusive accounting paths, eliminating duplicate spread
  and growth deltas
- generator replacement metrics also wait for the final block-form result and
  discard cancelled event identities without retaining them
- finality/inheritance regression coverage and the complete 182-task check
  passed

Release notes carried forward from `v1.1.193`:
...
- successful official `FurniturePlaceEvent` and `FurnitureBreakEvent` payloads
  immediately add or remove their stable `craftengine:<id>` value key
- bounded full reconciliation resolves the same ID through
  `CraftEngineFurniture.getLoadedFurnitureByMetaEntity(Entity)`, eliminating
  event-versus-rescan value drift
- generic entity delta paths now preserve custom furniture identity instead of
  collapsing recognized furniture to its vanilla entity carrier type
- official-shape event/API fixtures, exception fallback, integration gates,
  protection evidence, and the complete 182-task check passed

Release notes carried forward from `v1.1.191`:

- CraftEngine is now an executable custom-block identity integration across
  block deltas, bounded rescans, level, worth, ranking, and placement limits
- the adapter follows CraftEngine's stable
  `CraftEngineBlocks.getCustomBlockState(Block)` API and preserves configured
  IDs as `craftengine:<id>` instead of pricing the vanilla carrier material
- lookups use the real Bukkit `Block`; no synthetic location or nullable world
  conversion is introduced in block-change paths
- the integration remains optional and reflection-isolated; absent, changed,
  null, or failing APIs fall back to the vanilla key without breaking Paper
- policy, config, soft-dependency, runtime certification, official-shape test
  fixtures, exception fallback, and the complete 182-task check are verified

Release notes carried forward from `v1.1.190`:
...
- all dispenser outputs now fail closed at protection boundaries, including
  armor dispensing and zero-velocity events resolved from block facing
- arrows and other projectiles fired by an external block projectile source can
  no longer damage entities inside another island's protection domain
- one shared automation boundary policy consistently covers stable islands,
  wilderness, different islands, and either side of an active migration
- targeted Paper regressions plus the complete 182-task check certify the new
  transfer, dispense, and block-projectile protections

Release notes carried forward from `v1.1.188`:

- Core island info, owner/member queries, public listings, and administrator
  inspection now expose the independent authoritative `BORDER` upgrade instead
  of incorrectly copying the protection `SIZE`
- active `SIZE`, `ISLAND_SIZE`, `BORDER`, and `BORDER_SIZE` changes refresh the
  personal world border for every online player currently inside that island
- border visibility, color, warning distance, policy, and full flag reset events
  also refresh affected players without requiring movement or reconnection
- asynchronous Core events cross back onto the Paper scheduler before reading
  online players, locations, or world borders; per-island burst deduplication
  prevents redundant refresh scans
- targeted route and policy regressions plus the complete 182-task check certify
  the authoritative response contract and scheduler-safe live refresh path

Release notes carried forward from `v1.1.187`:

- active `SIZE` changes now atomically update Paper protection, level-scan, and
  snapshot bounds instead of leaving the activation-time region in memory
- activation, restore, reset, and both migration phases carry Core's
  authoritative island size; stale bundle manifests are reconciled before use
- shard cells now fail startup below 1024 blocks or without 512-block region
  alignment, preventing neighboring islands from sharing portable region files
- islands that exceed a cell's non-overlapping capacity fail before activation;
  unsafe live resizes preserve the last safe bounds and fence the island
- the migration state machine now permits the intended source-save transition
  from `DEACTIVATING` to target `ACTIVATING`, with regression coverage
- the complete 182-task check certifies the atomic region replacement, live
  resize listener, cell geometry, job propagation, and migration continuation

Release notes carried forward from `v1.1.186`:

- `/is deposit *` now deposits the player's complete current Vault balance, and
  `/is withdraw *` withdraws the island's complete authoritative bank balance
- wildcard transfers reuse the existing idempotent Core mutation, economy
  refund, payout, and rollback flows instead of introducing a separate unsafe
  transaction path
- zero balances stop before mutation, provider availability is checked first,
  and per-player pending-operation guards prevent overlapping wildcard requests
- tab completion advertises `*` together with common fixed amounts for English,
  Korean, and canonical bank command aliases
- targeted bank tests plus the complete 182-task check certify exact decimal
  transfer amounts, empty-balance behavior, scheduler-safe Vault reads, and Core
  rollback compatibility

Release notes carried forward from `v1.1.185`:

- island-node Paper servers now fail closed on player and non-player entity
  portals that originate inside an active island region, preventing unmanaged
  default Nether/End worlds from bypassing CloudIslands protection and routing
- blocked portal events disable destination portal creation and apply a bounded
  five-second entity cooldown, avoiding repeated portal searches and event spam
- players receive a localized, rate-limited action-bar explanation directing
  them to island home and warp commands; lobby and unrelated world portals are
  left untouched
- health JSON, Prometheus metrics, and heartbeat metadata expose the portal
  policy plus separate player/entity block counters for operator diagnosis
- Paper packaging metadata and regression tests certify both portal event paths,
  active-region scoping, unregister behavior, localization, and bootstrap wiring

Release notes carried forward from `v1.1.184`:

- island activation now reloads the authoritative persisted biome from Core and
  reapplies it to the restored Paper region, closing the inactive-admin/API
  mutation gap where metadata could previously diverge from the live world
- reconciliation keeps the Core request off the Paper main thread and returns
  to the scheduler only for Bukkit registry, world, chunk, and biome access
- a global monotonic generation token makes activation reconciliation, live
  biome changes, and deactivate/reactivate races deterministic: only the newest
  request can continue painting chunks
- deactivation cancels outstanding biome work before the region is released,
  while missing Core data or unavailable worlds fail visibly in the server log
- targeted environment tests and the complete 182-task check verify the new
  lifecycle path together with existing asynchronous chunk batching

Release notes carried forward from `v1.1.183`:

- safe MiniMessage rendering now covers the remaining Paper runtime surfaces:
  route and migration boss bars, action bars, command replies, warmup titles,
  login-session rejections, kick screens, boundary returns, flight/PVP notices,
  protection denials, and level recalculation notices
- `/ciadmin message` and `/ciadmin title` now accept the same formatting-only
  component syntax instead of exposing MiniMessage tags as literal text
- route boss-bar names refresh through Adventure components as progress changes,
  while localized player output still passes through physical-node privacy
  sanitization before rendering
- executable and data-reading MiniMessage tags remain inert on every new output
  path, preserving the command, URL, insertion, selector, score, and NBT safety
  boundary introduced in 1.1.182
- regression coverage scans all runtime presentation paths and behaviorally checks
  formatted fallbacks, safe runtime text, hostile placeholders, and inert commands

Release notes carried forward from `v1.1.182`:
...
- `/섬 border red|green|blue`, `/섬 border color <color>`, and
  ...
  of changing the island-wide `BORDER_COLOR` flag
- changing color automatically enables that player's border, matching the
  SuperiorSkyblock2 command contract
- Paper renders actual blue, green, and red client borders through stable or
  long-running grow/shrink transitions rather than reporting a color label that
  was never applied
- the modern tick-based Paper API is used when present, with a guarded legacy
  reflection fallback so older supported 1.21 runtimes do not fail linkage
- PostgreSQL V85 and MySQL/MariaDB V8 persist normalized colors, while Paper and
  Velocity commands and the border GUI share the same typed profile mutation

Release notes carried forward from `v1.1.179`:

- `/섬 toggle border`, `/섬 border-visible`, and `/섬 toggleblocks` now persist
  player-scoped preferences instead of changing an island-wide flag or limit for
  every member
- PostgreSQL V84 and MySQL/MariaDB V7 carry both visual preferences through the
  typed Core profile API across Paper nodes
- Paper refreshes its world border only when the player crosses an island-region
  boundary and clears only a border CloudIslands created, preserving borders from
  other plugins
- Paper and Velocity share the same typed player-profile mutations, and the border
  menu reports the requesting player's preference
- a dedicated release gate rejects regressions back to island-wide border or
  stacked-block display mutations

Release notes carried forward from `v1.1.178`:
...
- death, hanging-break, vehicle-destroy, and explosion causes stay on their
  dedicated accounting paths, preventing duplicate decrements
- direct plugin and internal discard removals are permanent and decrement both
  aggregate entity limits and exact per-type counters immediately
...
- supported Paper version: Paper `1.21.x` and stable `26.1.x` are
  release-supported; Paper `26.2.x` build 60 beta is compile- and boot-verified
  experimentally on Java 25 but is not release-supported until the stable
  Paper channel is available
